// ========================================
// print.js - دوال الطباعة المتقدمة (مع دعم CORS)
// ========================================
...
function printInvoice() {
    try { window.print(); printShowToast('تم إرسال الفاتورة إلى الطابعة', false); }
    catch (error) { printShowToast('حدث خطأ أثناء الطباعة', true); }
}

function previewPrint() {
    try {
        let pages = document.querySelectorAll('.page');
        if (pages.length === 0) { printShowToast('لا توجد صفحات للطباعة!', true); return; }
        let printWindow = window.open('', '_blank', 'width=800,height=600,scrollbars=yes,toolbar=yes');
        if (!printWindow) { printShowToast('الرجاء السماح بالنوافذ المنبثقة', true); return; }
        let pagesContent = '';
        ...
        let printContent = `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>معاينة الفاتورة</title>
            ...
            <style>body { background: #e9ecef; padding: 20px; } .page { margin: 0 auto 20px; box-shadow: 0 0 10px rgba(0,0,0,0.1); } @media print { body { background: white; padding: 0; } .page { margin: 0; box-shadow: none; } .no-print { display: none !important; } }
            ...
            .btn-png { background: #16a34a; color: white; } .btn-close { background: #6c757d; color: white; }</style>
    ...
    } catch (error) { printShowToast('حدث خطأ في المعاينة', true); }
}
...
async function exportToPDF() {
    ...
    if (typeof html2canvas === 'undefined' || typeof window.jspdf === 'undefined') {
        printShowToast('المكتبات غير محملة، حاول مرة أخرى', true); return;
    }
    ...
    try {
        ...
        for (let i = 0; i < pages.length; i++) {
            let canvas = await html2canvas(pages[i], { scale: 3, useCORS: true, backgroundColor: '#ffffff', logging: false });
            if (i !== 0) pdf.addPage();
    ...
    } catch(error) { printShowToast('خطأ في إنشاء PDF', true); }
    finally { if (buttons) buttons.style.display = 'flex'; printHideLoading(); }
}
...
async function exportToPNG() {
    ...
    try {
        for (let i = 0; i < pages.length; i++) {
            let canvas = await html2canvas(pages[i], { scale: 3, useCORS: true, backgroundColor: '#ffffff' });
    ...
    } catch(error) { printShowToast('خطأ في إنشاء PNG', true); }
